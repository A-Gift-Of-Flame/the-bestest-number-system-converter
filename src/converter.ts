...
export const BASES: BaseInfo[] = [
  { id: 'text',      label: 'Text (UTF-8)',   shortLabel: 'TEXT',      description: 'Plain text string' },
  { id: 'binary',    label: 'Binary',         shortLabel: 'BIN',       prefix: '0b', description: 'Base-2' },
  { id: 'octal',     label: 'Octal',          shortLabel: 'OCT',       prefix: '0o', description: 'Base-8' },
  { id: 'decimal',   label: 'Decimal',        shortLabel: 'DEC',       description: 'Base-10' },
  { id: 'hex',       label: 'Hexadecimal',    shortLabel: 'HEX',       prefix: '0x', description: 'Base-16' },
  { id: 'base32',    label: 'Base32',         shortLabel: 'B32',       description: 'RFC 4648' },
  { id: 'base64',    label: 'Base64',         shortLabel: 'B64',       description: 'Standard (RFC 4648)' },
  { id: 'base64url', label: 'Base64url',      shortLabel: 'B64U',      description: 'URL-safe (RFC 4648)' },
];
...
export function parseToBytes(value: string, base: Base): Uint8Array {
  ...
  switch (base) {
    ...
    case 'binary': {
      if (!stripped) throw new Error('Empty input');
      if (!/^[01]+$/.test(stripped)) throw new Error('Invalid binary — only 0 and 1 allowed');
    ...
    case 'octal': {
      if (!stripped) throw new Error('Empty input');
      if (!/^[0-7]+$/.test(stripped)) throw new Error('Invalid octal — only digits 0–7 allowed');
      return bigintToBytes(BigInt('0o' + stripped));
    }
    case 'decimal': {
      if (!stripped) throw new Error('Empty input');
      if (!/^\d+$/.test(stripped)) throw new Error('Invalid decimal — only digits 0–9 allowed');
      return bigintToBytes(BigInt(stripped));
    }
    case 'hex': {
      if (!stripped) throw new Error('Empty input');
      if (!/^[0-9a-fA-F]+$/.test(stripped)) throw new Error('Invalid hex — only 0–9 and A–F allowed');
...
function base64Decode(input: string, urlSafe: boolean): Uint8Array {
  ...
  for (let i = 0; i < stripped.length; i += 4) {
    ...
    if (a === -1 || b === -1) throw new Error(`Invalid Base64 character at position ${i}`);
    output.push((a << 2) | (b >> 4));
...
export function convertAll(value: string, base: Base): ConversionResult {
  ...
  for (const b of BASES) empty[b.id] = '';

  if (!value.trim() && base !== 'text') {
    return { bytes: null, error: null, outputs: empty as Record<Base, string>, bitLength: 0, byteLength: 0 };
  }
  ...
  try {
    ...
    return { bytes: null, error: (e as Error).message, outputs: empty as Record<Base, string>, bitLength: 0, byteLength: 0 };
  }

  const outputs: Record<string, string> = {};
  for (const b of BASES) {
    try { outputs[b.id] = bytesToBase(bytes, b.id); }
    catch { outputs[b.id] = ''; }
  }
  ...
  return {
    bytes,
    error: null,
    outputs: outputs as Record<Base, string>,
    bitLength: significantBits,
